...
public class ProductController {
    ...
    /**
     * Retrieves the total count of barcodes inserted in product_barcodes table for a product.
     *
     * @param id the product identifier
     * @return count of stored barcodes
     */
    @GetMapping("/{id}/barcode-count")
    @PreAuthorize("hasAnyRole('ADMIN', 'STORE_MANAGER', 'SALES_EXECUTIVE')")
    @Operation(summary = "Get barcode count for product", description = "Returns the count of generated barcodes stored in product_barcodes table for a given product ID.")
    public ResponseEntity<Long> getBarcodeCount(
            @Parameter(description = "ID of the product", required = true)
            @PathVariable Long id) {
        long count = productService.getBarcodeCountByProductId(id);
